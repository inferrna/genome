...
prg = cl.Program(ctx, """
  ...
  int merge(__global const int *arr1, int arr1size, __global const int *arr2, int arr2size,
  __global int *out) {
    int x = 0;
    int y = 0;
    int n = 0;
  ...
  int cpy(__global int *out, __global const int *in, int size) {
    int x = 0;
    for(x = 0; x < size; x++) {
      out[x] = in[x];
    }
...
start = datetime.now()
prg.mergesort(queue, (arrsize,), None, input_buf, dest_buf, len_buf)
end = datetime.now()
